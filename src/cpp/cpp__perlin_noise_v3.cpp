...
/*
 * generate_perlin_noise - generates a 2D Perlin noise map
 */
void generate_perlin_noise(const Options& opts) {
    int width = opts.width;
    int height = opts.height;
    float frequency = opts.frequency;   // higher = finer details
    float amplitude = opts.amplitude;   // higher = stronger contrast

    unsigned int channels = 1;
    std::vector<unsigned char> output(width * height * channels, 0);

    /* global gradient grid size */
    int grad_w = static_cast<int>(std::ceil(width  / static_cast<float>(CHUNK_SIDE_LENGTH))) + 2;
    int grad_h = static_cast<int>(std::ceil(height / static_cast<float>(CHUNK_SIDE_LENGTH))) + 2;
    ...
    /* generate continuous Perlin noise over the entire image */
